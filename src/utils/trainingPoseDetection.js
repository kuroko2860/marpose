...
const isDefenderFacingAttacker = (defenderPose, attackerPose) => {
  ...
  if (
    !defenderLeftShoulder ||
    !defenderRightShoulder ||
    !attackerLeftShoulder ||
    !attackerRightShoulder
  )
    return false;
  ...
  const defenderLeftToAttacker = calculateDistance(
    defenderLeftShoulder,
    attackerCenter
  );
  const defenderRightToAttacker = calculateDistance(
    defenderRightShoulder,
    attackerCenter
  );
...
export const detectArmCrossing = (defenderPose, attackerPose = null) => {
  ...
  if (attackerPose) {
    ...
    const defenderToAttackerDistance = calculateRelativeDistance(
      defenderPose,
      attackerPose,
      9,
      9
    ...
    const leftWristToAttackerWrist = calculateRelativeDistance(
      defenderPose,
      attackerPose,
      9,
      9
    ); // Cổ tay trái người phòng thủ đến cổ tay trái kẻ tấn công
    const rightWristToAttackerWrist = calculateRelativeDistance(
      defenderPose,
      attackerPose,
      10,
      10
    ); // Cổ tay phải người phòng thủ đến cổ tay phải kẻ tấn công
    ...
    if (leftWristToAttackerWrist) {
      if (leftWristToAttackerWrist <= 70) {
        ...
        leftWristScore = Math.max(
          0,
          1.0 - (leftWristToAttackerWrist - 70) / maxDistance
        );
      }
    }
    ...
    if (rightWristToAttackerWrist) {
      if (rightWristToAttackerWrist <= 70) {
        ...
        rightWristScore = Math.max(
          0,
          1.0 - (rightWristToAttackerWrist - 70) / maxDistance
        );
      }
    }
    ...
    const leftAnkleToAttackerAnkle = calculateRelativeDistance(
      defenderPose,
      attackerPose,
      15,
      16
    ); // Mắt cá chân trái người phòng thủ đến mắt cá chân trái kẻ tấn công
    const rightAnkleToAttackerAnkle = calculateRelativeDistance(
      defenderPose,
      attackerPose,
      16,
      15
    ); // Mắt cá chân phải người phòng thủ đến mắt cá chân phải kẻ tấn công
    ...
    if (leftAnkleToAttackerAnkle) {
      if (leftAnkleToAttackerAnkle <= 70) {
        ...
        // Tính điểm giảm dần khi xa khỏi phạm vi mục tiêu
        const maxDistance = 150; // Khoảng cách tối đa để có điểm
        ankleDistanceScore = Math.max(
          0,
          1.0 - (leftAnkleToAttackerAnkle - 70) / maxDistance
        );
      }
      ...
      if (rightAnkleToAttackerAnkle <= 70) {
        ...
        ankleDistanceScore = Math.max(
          0,
          1.0 - (rightAnkleToAttackerAnkle - 70) / maxDistance
        );
      }
    }
    ...
    const leftWristToAttackerElbow = calculateRelativeDistance(
      defenderPose,
      attackerPose,
      10,
      7
    ); // Cổ tay trái người phòng thủ đến khuỷu tay trái kẻ tấn công
    const rightWristToAttackerElbow = calculateRelativeDistance(
      defenderPose,
      attackerPose,
      9,
      8
    ); // Cổ tay phải người phòng thủ đến khuỷu tay phải kẻ tấn công
    ...
    if (leftWristToAttackerElbow) {
      if (leftWristToAttackerElbow <= 100) {
        ...
        leftElbowScore = Math.max(
          0,
          1.0 - (leftWristToAttackerElbow - 100) / maxDistance
        );
      }
    }
    ...
    if (rightWristToAttackerElbow) {
      if (rightWristToAttackerElbow <= 100) {
        ...
        rightElbowScore = Math.max(
          0,
          1.0 - (rightWristToAttackerElbow - 100) / maxDistance
        );
      }
    }
    ...
    relativeFeedback = {
      ...
      elbowCuttingScore: `Cắt khuỷu tay: ${(elbowCuttingScore * 100).toFixed(
        0
      )}%`,
      leftElbowScore: `Cổ tay trái cắt: ${(leftElbowScore * 100).toFixed(0)}%`,
      rightElbowScore: `Cổ tay phải cắt: ${(rightElbowScore * 100).toFixed(
        0
      )}%`,
      isHoldingAttackerWrist: isHoldingAttackerWrist
        ? "✅ Nắm cổ tay đối thủ đúng"
        : "❌ Cần nắm cổ tay đối thủ gần hơn",
      isLegOnAttackerLeg: isLegOnAttackerLeg
        ? "✅ Chân đè lên chân đối thủ đúng"
        : "❌ Cần đè chân lên chân đối thủ",
      isCuttingAtAttackerElbow: isCuttingAtAttackerElbow
        ? "✅ Tay cắt ở khuỷu tay đối thủ đúng"
        : "❌ Cần cắt tay ở khuỷu tay đối thủ",
    };
  }
...
export const detectHoldingLeg = (defenderPose, attackerPose = null) => {
  ...
  if (attackerPose) {
    ...
    if (leftAnkleToAttackerAnkle) {
      if (leftAnkleToAttackerAnkle >= 100 && leftAnkleToAttackerAnkle <= 150) {
        ...
        const distanceFromTarget = Math.abs(
          leftAnkleToAttackerAnkle - targetCenter
        );
        const maxDistance = 100; // Khoảng cách tối đa để có điểm
        ankleDistanceScore = Math.max(
          0,
          1.0 - distanceFromTarget / maxDistance
        );
      }
    } else if (rightAnkleToAttackerAnkle) {
      if (
        rightAnkleToAttackerAnkle >= 100 &&
        rightAnkleToAttackerAnkle <= 150
      ) {
        ankleDistanceScore = 1.0; // Điểm đầy đủ trong phạm vi mục tiêu
      } else {
        const targetCenter = 125;
        const distanceFromTarget = Math.abs(
          rightAnkleToAttackerAnkle - targetCenter
        );
    ...
    const leftWristToAttackerAnkle = calculateRelativeDistance(
      defenderPose,
      attackerPose,
      9,
      15
    ); // Cổ tay trái người phòng thủ đến mắt cá chân trái kẻ tấn công
    const rightWristToAttackerAnkle = calculateRelativeDistance(
      defenderPose,
      attackerPose,
      10,
      16
    ); // Cổ tay phải người phòng thủ đến mắt cá chân phải kẻ tấn công
    ...
    if (leftWristToAttackerAnkle) {
      if (leftWristToAttackerAnkle <= 40) {
        ...
        leftWristScore = Math.max(
          0,
          1.0 - (leftWristToAttackerAnkle - 40) / maxDistance
        );
      }
    }
    ...
    if (rightWristToAttackerAnkle) {
      if (rightWristToAttackerAnkle <= 40) {
        ...
        rightWristScore = Math.max(
          0,
          1.0 - (rightWristToAttackerAnkle - 40) / maxDistance
        );
      }
    }
    ...
    relativeFeedback = {
      ankleDistanceScore: `Khoảng cách chân: ${(
        ankleDistanceScore * 100
      ).toFixed(0)}%`,
      leftWristScore: `Cổ tay trái: ${(leftWristScore * 100).toFixed(0)}%`,
      rightWristScore: `Cổ tay phải: ${(rightWristScore * 100).toFixed(0)}%`,
      wristDistanceScore: `Trung bình tay: ${(wristDistanceScore * 100).toFixed(
        0
      )}%`,
      ankleDistanceInRange: ankleDistanceInRange
        ? "✅ Khoảng cách chân đúng"
        : "❌ Cần điều chỉnh khoảng cách chân",
      isWristCloseToAttackerAnkle: isWristCloseToAttackerAnkle
        ? "✅ Tay gần mắt cá chân đối thủ đúng"
        : "❌ Cần đưa tay gần mắt cá chân đối thủ hơn",
 ...
 */
export const detectTrainingPose = (
  defenderPose,
  trainingTypeId,
  attackerPose = null
) => {
 ...
 */
export const analyzePoseCorrectness = (
  defenderPose,
  trainingTypeId,
  attackerPose = null
) => {
  const detection = detectTrainingPose(
    defenderPose,
    trainingTypeId,
    attackerPose
  );
